  ...
  return (
    <div className="flex h-screen bg-black text-white font-sans">
      ...
      <div className="w-64 border-r border-white/10 bg-[#0a0a0a] flex flex-col p-2 gap-1">
        ...
        <NavBtn act={activeTab === "mmbe"} icon={<Box size={18}/>} label="MMBE: Остатки" onClick={() => setActiveTab("mmbe")} />
        <NavBtn act={activeTab === "migo"} icon={<ArrowDownToLine size={18}/>} label="MIGO: Приход" onClick={() => setActiveTab("migo")} />
        <NavBtn act={activeTab === "mb1b"} icon={<ArrowRightLeft size={18}/>} label="MB1B: Перенос" onClick={() => setActiveTab("mb1b")} />
        <NavBtn act={activeTab === "mi01"} icon={<Calculator size={18}/>} label="MI01: Подсчет" onClick={() => setActiveTab("mi01")} />
        <NavBtn act={activeTab === "mb51"} icon={<History size={18}/>} label="MB51: Журнал" onClick={() => setActiveTab("mb51")} />
      </div>
