...
export default function CashierPage() {
  ...
  useEffect(() => {
    ...
    if (saved) { try { setCashierSettings(prev => ({ ...prev, ...JSON.parse(saved) })); } catch {} }
  ...
  useAutoLock({ timeoutMinutes: cashierSettings.autoLockMinutes, enabled: cashierSettings.autoLockEnabled && !!session && !isLocked, onLock: ()=>setIsLocked(true) });

  const [appliedDiscount, setAppliedDiscount] = useState<{id:string,name:string,type:'percent'|'fixed',value:number,reason?:string}|null>(null);

  const broadcastToCustomerDisplay=(items:CartItem[],subtotalVal:number,discountVal:number,totalVal:number)=>{try{const channel=new BroadcastChannel('customer_display');channel.postMessage({type:'cart_update',data:{items:items.map(ci=>({id:ci.menuItem.id,name:ci.menuItem.name,quantity:ci.quantity,price:Number(ci.menuItem.price)})),subtotal:subtotalVal,discount:discountVal,total:totalVal}});channel.close()}catch{}}
  ...
  const openShift=async(cashierSession:CashierSession)=>{try{
    const {data:existingShift}=await supabase.from("shifts").select("id, started_at").eq("user_id",cashierSession.id).eq("location_id",cashierSession.location_id).is("ended_at",null).maybeSingle();
    if(existingShift){const updatedSession={...cashierSession,shift_id:existingShift.id,shift_start:existingShift.started_at};setSession(updatedSession);sessionStorage.setItem("cashier_session",JSON.stringify(updatedSession));toast.info("Восстановлена открытая смена");return;}
    const {data,error}=await supabase.from("shifts").insert({user_id:cashierSession.id,location_id:cashierSession.location_id}).select().single();
  ...
  const itemsByCategory = useMemo(()=>{const map=new Map<string,MenuItem[]>();menuItems.forEach(item=>{const catId=item.category_id;if(!map.has(catId)) map.set(catId,[]);map.get(catId)!.push(item);});return map},[menuItems]);
  const getCategoryStyle=(name:string)=>categoryStyles[name]||defaultCategoryStyle;

  const addToCart=async(item:MenuItem)=>{
    if(!cashierSettings.allowNegativeStock && session){try{
      ...
      if(recipe?.length){for(const r of recipe){if(r.ingredient_id){const {data:inv}=await supabase.from("inventory").select("quantity").eq("location_id",session.location_id).eq("ingredient_id",r.ingredient_id).maybeSingle();const currentQty=inv?Number(inv.quantity):0;const existingInCart=cart.find(ci=>ci.menuItem.id===item.id);const cartQty=existingInCart?existingInCart.quantity:0;if(currentQty<Number(r.quantity)*(cartQty+1)){toast.error(`Недостаточно остатков для "${item.name}"`);return;}}}}}catch(e){console.error(e);}
    }
    playCartAddSound();
  ...
  const handleCloseShift=async()=>{
    ...
    try{
      ...
      toast.success(`Смена закрыта. Рабочее время: ${session.shift_start?Math.round((Date.now()-new Date(session.shift_start).getTime())/60000):0} мин`);
    }catch(e){console.error(e);}finally{handleLogout();}
  };

  const handleLogout=()=>{sessionStorage.removeItem("cashier_session");navigate("/pin");};
    ...
    <div className="flex-1 flex flex-col">
      <header className="h-16 border-b bg-card flex items-center justify-between px-6 shadow-sm gap-4">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center shadow-lg"><UtensilsCrossed className="text-primary-foreground h-5 w-5"/></div>
          <div>
            <p className="font-semibold">{session?.full_name}</p>
            <p className="text-xs text-muted-foreground">{t('cashier.title')} • {session?.shift_id?'Смена открыта':'Смена не открыта'} {fromCache && <span className="ml-1 text-amber-500">(кэш)</span>}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {lastOrder && <Button variant="ghost" size="sm" onClick={()=>setReceiptDialogOpen(true)} className="gap-2"><Printer className="h-4 w-4"/> #{lastOrder.orderNumber}</Button>}
          <LanguageSelector />
          <Button variant="ghost" size="sm" onClick={()=>setIsLocked(true)} className="gap-2"><Lock className="h-4 w-4"/>{t('cashier.lock')}</Button>
          <Button variant="outline" size="sm" onClick={()=>setRefundDialogOpen(true)} className="gap-2"><RotateCcw className="h-4 w-4"/>{t('cashier.refund')}</Button>
          <Button variant="outline" size="sm" onClick={handleCloseShift} className="gap-2 text-destructive hover:text-destructive"><Clock className="h-4 w-4"/> Закрыть смену</Button>
          <Button variant="ghost" size="sm" onClick={handleLogout} className="gap-2"><LogOut className="h-4 w-4"/></Button>
        </div>
      </header>
      <ScrollArea className="flex-1 p-6">
        {!selectedCategory?<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">{categories.map(cat=>{const style=getCategoryStyle(cat.name);const Icon=style.icon;const count=itemsByCategory.get(cat.id)?.length||0;return (<Card key={cat.id} className={cn("cursor-pointer",style.bg)} onClick={()=>setSelectedCategory(cat.id)}><CardContent className="p-4 text-center"><Icon className={cn("h-7 w-7 mx-auto mb-2",style.color)}/><p>{cat.name}</p><p className="text-sm text-muted-foreground">{count} позиций</p></CardContent></Card>);})}</div>:<div><Button variant="ghost" size="sm" onClick={()=>setSelectedCategory(null)}>← Назад</Button><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mt-4">{itemsByCategory.get(selectedCategory)?.map(item=>(<Card key={item.id} className="cursor-pointer overflow-hidden" onClick={()=>addToCart(item)}>{item.image_url?(<div className="aspect-square bg-muted"><img src={item.image_url} alt={item.name} className="w-full h-full object-cover" onError={(e)=>{(e.target as HTMLImageElement).style.display='none';}}/></div>):<div className="w-12 h-12 bg-muted rounded-lg flex items-center justify-center mx-auto mb-2"><Image className="h-6 w-6 text-muted-foreground"/></div>}<CardContent className={cn("p-3",!item.image_url&&"pt-6")}><p className="font-medium text-center line-clamp-2">{item.name}</p><p className="font-bold text-center text-primary">{Number(item.price).toLocaleString()} ֏</p
