...
export default function PinLogin() {
  ...
  /* ===== ЗАГРУЗКА ЛОКАЦИЙ (БЕЗ ЦЕНТРАЛЬНОГО) ===== */
  useEffect(() => {
    const fetchLocations = async () => {
      const { data } = await supabase
        ...
        .select("id, name")
        .eq("is_active", true)
        .neq("name", "Центральный"); // Фильтр: не показывать Центральный
  ...
  const handlePinSubmit = async () => {
    ...
    try {
      ...
      if (funcError) {
        ...
        showError(details.message || "Ошибка авторизации", isShiftError ? "warning" : "error");
        return;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-[#030308]">
      {/* Mesh Background */}
      ...
      <div className={`relative z-10 w-full max-w-sm transition-all duration-500 ${isShaking ? "animate-shake" : ""}`}>
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="relative group mb-4">
            ...
            <div className="relative w-20 h-20 bg-white/[0.03] backdrop-blur-3xl rounded-[2.2rem] border border-white/10 flex items-center justify-center shadow-2xl">
              <LockKeyhole className="text-indigo-400 h-9 w-9 drop-shadow-[0_0_8px_rgba(129,140,248,0.6)]" />
            </div>
          </div>
          <h1 className="text-white font-black text-2xl tracking-tight uppercase">
            Crusty <span className="text-indigo-400 font-light italic">Point</span>
          </h1>
          <div className="flex items-center gap-2 mt-2 bg-white/5 px-4 py-1 rounded-full border border-white/5 backdrop-blur-md">
            <Sparkles className="h-3 w-3 text-indigo-400" />
            <span className="text-white/40 text-[9px] uppercase tracking-[0.3em] font-bold">Secure Terminal v2.0</span>
          </div>
        </div>
          ...
          <div
            ...
            ${
              errorType === "error"
                ...
                : "bg-orange-500/10 border-orange-500/30 text-orange-50"
            }
          `}
          >
            ...
            <div className="flex flex-col">
              <span className="text-[9px] uppercase tracking-widest opacity-40 font-black">Уведомление системы</span>
        ...
        {/* MAIN CARD */}
        <div className="backdrop-blur-[80px] bg-white/[0.02] border border-white/10 rounded-[3.5rem] p-8 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.7)]">
          {/* LOCATION SELECT */}
          <div className="mb-8">
            <Select value={selectedLocation} onValueChange={setSelectedLocation}>
              <SelectTrigger className="bg-white/5 border-none text-white/90 h-16 rounded-2xl shadow-inner px-6 hover:bg-white/10 transition-all focus:ring-1 focus:ring-indigo-500/30">
              ...
              <SelectContent className="bg-[#0a0a0f]/95 backdrop-blur-3xl border-white/10 text-white rounded-3xl p-2">
                {locations.map((loc) => (
                  <SelectItem
                    key={loc.id}
                    value={loc.id}
                    className="rounded-xl py-3 focus:bg-indigo-500/20 transition-all"
                  >
                    {loc.name}
                  </SelectItem>
                ))}
              ...
              >
                <div
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${
                    pin.length > i ? "bg-indigo-400 scale-100 shadow-[0_0_10px_#818cf8]" : "bg-white/5 scale-50"
                  }`}
                />
              </div>
            ))}
          ...
          {/* LOADING STATE */}
          <div
            className={`mt-8 flex flex-col items-center gap-2 transition-opacity duration-300 ${loading ? "opacity-100" : "opacity-0"}`}
          >
        ...
        <p className="text-center text-white/10 text-[9px] mt-10 tracking-[0.4em] uppercase font-black">
      ...
      <style
        ...
        /* Disable standard focus outlines */
