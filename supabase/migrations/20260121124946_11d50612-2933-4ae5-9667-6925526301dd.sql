-- Update the calculate_menu_item_cost function to include semi-finished products
...
BEGIN
    ...
    SELECT COALESCE(SUM(
        mii.quantity * (
            SELECT COALESCE(SUM(sfi.quantity * ing.cost_per_unit), 0) / NULLIF(sf.output_quantity, 0)
